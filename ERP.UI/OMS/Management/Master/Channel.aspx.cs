...
{
    public partial class Channel : ERP.OMS.ViewState_class.VSPage
    {
        ...
        {
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSession("/management/Master/Channel.aspx");
            ...
            if (HttpContext.Current.Session["EntryProfileType"] != null)
            {
                ...
                {
                    channel.ConnectionString = ConfigurationSettings.AppSettings["DBReadOnlyConnection"];
                }
                else
                {
                    channel.ConnectionString = ConfigurationSettings.AppSettings["DBConnectionDefault"];
                }
            }
        ...
        {
            string filename = "Channel List";
            exporter.FileName = filename;

            exporter.PageHeader.Left = "Channel list";
        ...
        protected void DesigGrid_RowDeleting(object sender, DevExpress.Web.Data.ASPxDataDeletingEventArgs e)
        {
            ...
            int i = masterdata.DeleteChannel(Convert.ToString(id));
            if (i == 1)
            {
        ...
        protected void DesigGrid_StartRowEditing(object sender, DevExpress.Web.Data.ASPxStartRowEditingEventArgs e)
        {
            ...
            DesigGrid.SettingsText.PopupEditFormCaption = "Edit Channel";
            int id = Convert.ToInt32(Convert.ToString(e.EditingKeyValue));
            Session["Desigid"] = id;
        }
        //Purpose: Add Edit and delete rights to Channel
        //Name: Debjyoti Dhar.
        ...
        public static bool CheckUniqueCode(string ProductClassCode, string proclassid)
        {
            ...
            {
                status = objMShortNameCheckingBL.CheckUnique(ProductClassCode.Trim(), proclassid, "MasterChannel");
            }
            return status;
        }
        ...
        //Mantis Issue 25017
        [WebMethod]
        public static List<StageList> GetDSTypeList(string CHNLID)
        {
            BusinessLogicLayer.DBEngine objEngine = new BusinessLogicLayer.DBEngine();
            List<StageList> omodel = new List<StageList>();
            DataTable dt = new DataTable();
            ProcedureExecute proc = new ProcedureExecute("PROC_USER_ChannelDSTypeMap");
            proc.AddPara("@CHNLID", CHNLID);
            dt = proc.GetTable();
            if (dt != null && dt.Rows.Count > 0)
            {
                omodel = UtilityLayer.APIHelperMethods.ToModelList<StageList>(dt);
            }
            return omodel;
        }

        [WebMethod]
        public static bool GetStagelistSubmit(string chnlID, List<string> Stagelist)
        {
            ...
            DataTable dt = new DataTable();
            ProcedureExecute proc = new ProcedureExecute("PRC_ChannelStageInsertUpdate");
            proc.AddPara("@chnlID", Convert.ToInt32(chnlID));
            proc.AddPara("@Stagelist", StageId);
            proc.AddPara("@User_id", Convert.ToString(HttpContext.Current.Session["userid"]));
            dt = proc.GetTable();
            //DataTable dtfromtosumervisor = objEmploye.SubmitEmployeeBranch(chnlID, StageId, Convert.ToString(HttpContext.Current.Session["userid"]));
