...
{
    public class HomeVisitRegisterController : Controller
    {
        //
        // GET: /MYSHOP/HomeVisitRegister/
        public ActionResult Index()
        {
            return View();
        }

        public ActionResult GetHomeVisitRegisterPartial(BusinessLogicLayer.SalesmanTrack.HomeVisitRegisterModel model)
        {
            HomeVisitRegister objshop = new HomeVisitRegister();
            try
            {
                ...
                if (days <= 7)
                {
                    if (model.Ispageload != "0")
                    {
                        dt = objshop.GetHomeVisitRegister(Employee, FromDate, ToDate, Convert.ToString(userID), State_id, Designation_id);
                    }
                }
                TempData["dt"] = dt;
                TempData.Keep();
                return PartialView("_PartialHomeVisitRegister", dt);
            }
            catch
            {
