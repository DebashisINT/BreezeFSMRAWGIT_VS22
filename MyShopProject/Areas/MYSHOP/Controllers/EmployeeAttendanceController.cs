...
namespace MyShop.Areas.MYSHOP.Controllers
{
    public class EmployeeAttendanceController : Controller
    {
        ...
        public PartialViewResult EmployeeAttendanceGridViewCallback(EmployeeMasterReport model)
        {
            ...
            if (model.is_pageload != "0" && model.is_pageload != null)
            {
                ...
                {
                    ProcedureExecute proc = new ProcedureExecute("PRC_FTSEMPLOYEEATTENDANCE_REPORT");
            ...
            TempData["EnployeeMastergridView"] = ds;
        ...
        public ActionResult ExportAttendanceGridView(int type)
        {
            ...
            ViewData["EnployeeMastergridView"] = TempData["EnployeeMastergridView"];

            DataSet DS = (DataSet)ViewData["EnployeeMastergridView"];

            //End Rev
            TempData.Keep();

            if (ViewData["EnployeeMastergridView"] != null)
            {

                switch (type)
                {
                    case 1:
                        return GridViewExtension.ExportToPdf(GetDashboardGridView(ViewData["EnployeeMastergridView"]), DS.Tables[1]);
                    //break;
                    case 2:
                        //return GridViewExtension.ExportToXlsx(GetDashboardGridView(ViewData["EnployeeMastergridView"]), DS.Tables[1]);
                        return GridViewExtension.ExportToXlsx(GetDashboardGridView(ViewData["EnployeeMastergridView"]), DS.Tables[1], new XlsxExportOptionsEx() { ExportType = ExportType.WYSIWYG });
                    //break;
                    case 3:
                        return GridViewExtension.ExportToXls(GetDashboardGridView(ViewData["EnployeeMastergridView"]), DS.Tables[1]);
                    //break;
                    case 4:
                        return GridViewExtension.ExportToRtf(GetDashboardGridView(ViewData["EnployeeMastergridView"]), DS.Tables[1]);
                    //break;
                    case 5:
                        return GridViewExtension.ExportToCsv(GetDashboardGridView(ViewData["EnployeeMastergridView"]), DS.Tables[1]);
                    default:
                        break;
                }
            }
        ...
        private GridViewSettings GetDashboardGridView(object dataset)
        {
            ...
            settings.Name = "Employee Attendance Report";
            settings.SettingsExport.ExportedRowType = GridViewExportedRowType.All;
            settings.SettingsExport.FileName = "Employee Attendance Report";
            String ID = Convert.ToString(TempData["EnployeeMastergridView"]);
            TempData.Keep();
